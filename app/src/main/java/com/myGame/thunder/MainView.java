...
public class MainView extends View {
    ...
    public MainView(Context context) {
        super(context);
/*		try {
            filter=Bitmap.createScaledBitmap(BitmapFactory.decodeStream(getAssets().open("Filter.png")), size, size,
			true);
		} catch (IOException e) {
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (MainActivity.begin && ani) {
            ...
            drawBoss(paint, canvas, MainActivity.boss);
...
/*	public boolean onTouchEvent(MotionEvent event){
    ...
    public void drawBoss(Paint paint, Canvas canvas, ArrayList<Boss> boss) {
        for (int m = 0; m < boss.size(); m++) {
            if (!boss.get(m).dead) {
                for (int i = 0; i < boss.get(m).getPixel().length; i++) {
                    if (!boss.get(m).getPixel()[i].isOutOfFrame()) {
                        //					System.out.println("draw");
                        canvas.drawRect(boss.get(m).getPixel()[i].getRect(), paint);
                    }
                }
                if (boss.get(m).ImmuneTime == 0) {
                    for (int n = boss.get(m).statusNumMax; n >= 0; n--) {
                        //		System.out.println(n);
                        paint.setColor(color[n % 2]);
                        for (int i = 0; i < 25 * (float) ((float) boss.get(m).life[n] / (float) boss.get(m)
								.totalLife[n]); i++) {
    ...
    public void drawGameOver(Paint paint, Canvas canvas) {
        ...
        for (int i = 0; i < alphabet.G().length; i++) {
            canvas.drawRect(new Pixel(alphabet.G()[i].x + 1, alphabet.G()[i].y + 5).getRect(), paint);
        }
        for (int i = 0; i < alphabet.A().length; i++) {
            canvas.drawRect(new Pixel(alphabet.A()[i].x + 7, alphabet.A()[i].y + 5).getRect(), paint);
        }
        for (int i = 0; i < alphabet.M().length; i++) {
            canvas.drawRect(new Pixel(alphabet.M()[i].x + 13, alphabet.M()[i].y + 5).getRect(), paint);
        }
        for (int i = 0; i < alphabet.E().length; i++) {
            canvas.drawRect(new Pixel(alphabet.E()[i].x + 19, alphabet.E()[i].y + 5).getRect(), paint);
        }
        for (int i = 0; i < alphabet.O().length; i++) {
            canvas.drawRect(new Pixel(alphabet.O()[i].x + 1, alphabet.O()[i].y + 13).getRect(), paint);
        }
        for (int i = 0; i < alphabet.V().length; i++) {
            canvas.drawRect(new Pixel(alphabet.V()[i].x + 7, alphabet.V()[i].y + 13).getRect(), paint);
        }
        for (int i = 0; i < alphabet.E().length; i++) {
            canvas.drawRect(new Pixel(alphabet.E()[i].x + 13, alphabet.E()[i].y + 13).getRect(), paint);
        }
        for (int i = 0; i < alphabet.R().length; i++) {
            canvas.drawRect(new Pixel(alphabet.R()[i].x + 19, alphabet.R()[i].y + 13).getRect(), paint);
        }
    }
    ...
    public void drawGameBegin(Paint paint, Canvas canvas) {
        ...
        for (int i = 0; i < alphabet.G().length; i++) {
            canvas.drawRect(new Pixel(alphabet.G()[i].x + 6, alphabet.G()[i].y + 9).getRect(), paint);
        }

        for (int i = 0; i < alphabet.O().length; i++) {
            canvas.drawRect(new Pixel(alphabet.O()[i].x + 12, alphabet.O()[i].y + 9).getRect(), paint);
        }
        for (int i = 0; i < alphabet.exclamation().length; i++) {
            canvas.drawRect(new Pixel(alphabet.exclamation()[i].x + 17, alphabet.exclamation()[i].y + 9).getRect(), paint);
        }
        for (int i = 0; i < alphabet.exclamation().length; i++) {
            canvas.drawRect(new Pixel(alphabet.exclamation()[i].x + 19, alphabet.exclamation()[i].y + 9).getRect(), paint);
        }
    }
}